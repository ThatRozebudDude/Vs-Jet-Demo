...
class ConfigMenu extends MusicBeatState
{
	...
	{	
		...
		if(startSong)
			FlxG.sound.playMusic('assets/music/JetConfig.ogg');
		else
			startSong = true;

		persistentUpdate = persistentDraw = true;

		var bg:FlxSprite = new FlxSprite(-320).loadGraphic(Paths.image("jet/menu/OptionsBG"));
		add(bg);

		abot = new FlxSprite(712, 4).loadGraphic(Paths.image("jet/menu/abot"));
		add(abot);

		radio = new FlxSprite(-88, 42).loadGraphic(Paths.image("jet/menu/radio"));
		add(radio);

		abot.x += abot.width;
		FlxTween.tween(abot, {x: abot.x - abot.width}, 1.2, {ease: FlxEase.quintOut});

		radio.x -= radio.width;
		FlxTween.tween(radio, {x: radio.x + radio.width}, 1.2, {ease: FlxEase.quintOut});
		...
		var optionTitle:FlxSprite = new FlxSprite(0, 10);
		optionTitle.frames = Paths.getSparrowAtlas("jet/menu/menuButtons");
				...
				switch(configSelected){
					case 0: //Offset
						...
						if(FlxG.keys.justPressed.ENTER){
							...
							Config.write(offsetValue, accuracyType, healthValue / 10.0, healthDrainValue / 10.0, true, downValue, glowValue, randomTapValue, noCapValue, scheme);
	...
	function exit(){
		...
		FlxTween.tween(abot, {x: abot.x + abot.width}, 0.6, {ease: FlxEase.quartIn});
		FlxTween.tween(radio, {x: radio.x - radio.width}, 0.6, {ease: FlxEase.quartIn});
