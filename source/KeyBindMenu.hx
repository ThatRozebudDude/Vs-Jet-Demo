...
class KeyBindMenu extends MusicBeatState
{
	...
	{	
		...
		var bg:FlxSprite = new FlxSprite(-320).loadGraphic(Paths.image("jet/menu/KeyBG"));
		add(bg);

		var abot = new FlxSprite(712, 4).loadGraphic(Paths.image("jet/menu/abotPurple"));
		add(abot);

		var radio = new FlxSprite(-88, 42).loadGraphic(Paths.image("jet/menu/radioPurple"));
		add(radio);
